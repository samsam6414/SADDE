function [Leader_pos_init,bestmem,bestval,nfeval] = SADDE(fname,VTR,D,XVmin,XVmax,y,NP,itermax,F,factor_method,F_cr,factor_method_cr,CR,strategy,refresh);
...
% factor_method  取比例因子方式
% F_cr          第五種方法特有比例因子
% factor_method_cr 第五種方法取特有比例因子方式
...
ind = zeros(4);
regbestmemit=zeros(itermax-1,D);
objf=zeros(itermax-1,1);
...
%end-----------------------------------------------------------------------
  ...
  elseif (st == 2)                  % DE/rand/1
    ui = popold + F*(bm - popold + pm1 - pm2);  % differential variation
    ui = popold.*mpo + ui.*mui;     % crossover
  elseif (st == 3)                  % DE/rand-to-best/1
    ui = popold + F*(bm-popold) + F*(pm1 - pm2);        
    ui = popold.*mpo + ui.*mui;     % crossover 
  elseif (st == 4)                  % DE/best/2
    ui = popold.*0.6 + bm.*0.4 + F*(bm - popold + pm1 - pm2 + pm3 - pm4);  % differential variation
    ui = popold.*mpo + ui.*mui;           % crossover
  elseif (st == 5)                  % DE/rand/2
    if rand<0.1
        F=0.1+rand*0.9;        
    end
    if factor_method_cr==1
        F_cr=rand;
    end
    ui = popold + F*(bm - popold)+F_cr*(pm1 - pm2);  % differential variation
    ui = popold.*mpo + ui.*mui;            % crossover
  end
...
%----Output section----------------------------------------------------------
  ...
  save everyitbest regbestmemit; 
  save objf objf; 
  iter = iter + 1;
  
  if iter ==2
     Leader_pos_init =  bestmem;
  end
